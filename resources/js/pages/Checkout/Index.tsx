...
export default function CheckoutIndex({ total }: { total: number }): JSX.Element {
    ...
    return (
        <main className="min-h-screen bg-zinc-950 px-6 py-12 text-zinc-100">
            ...
            <form onSubmit={(e) => { e.preventDefault(); post('/checkout'); }} className="mt-6 grid max-w-2xl gap-3">
                {Object.entries(data).map(([key, value]) => (
                    <input key={key} value={value} onChange={(e) => setData(key as any, e.target.value)} className="rounded border border-zinc-700 bg-zinc-900 p-3" placeholder={key} />
                ))}
